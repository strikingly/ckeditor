...
(function(){if(window.CKEDITOR&&window.CKEDITOR.dom)return;if(!window.CKEDITOR){window.CKEDITOR=(function(){var basePathSrcPattern=/(^|.*[\\\/])ckeditor\.js(?:\?.*|;.*)?$/i;var CKEDITOR={timestamp:'H5G9',version:'4.5.3 DEV',revision:'e4ae8da7e',rnd:Math.floor(Math.random()*(999-100+1))+100,_:{pending:[],basePathSrcPattern:basePathSrcPattern},status:'unloaded',basePath:(function(){var path=window.CKEDITOR_BASEPATH||'';if(!path){var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){var match=scripts[i].src.match(basePathSrcPattern);if(match){path=match[1];break;}}}
...
var status={nowType:'',};function renderLineGroupBlock(shouldAddRemoveBtn,nowType){var editor=this;var ulstyle=CKEDITOR.skin.getIconStyle('bulletedlist',false),olstyle=CKEDITOR.skin.getIconStyle('numberedlist',false);var clickFn=CKEDITOR.tools.addFunction(function(type){switch(type){case'ul':editor.execCommand('bulletedlist');break;case'ol':editor.execCommand('numberedlist');break;}})
reHtml='<a class="cke_button ck_btn_with_gray_border_top" onclick="CKEDITOR.tools.callFunction('+clickFn+', \'ul\')" hidefocus=true data-l="ul" style="float: left;outline: none;" title="'+editor.lang.list.bulletedlist+'"><span class="cke_button_icon cke_button__bulletedlist_icon" style="'+ulstyle+'"></span></a>';reHtml+='<a class="cke_button ck_btn_with_gray_border_top" onclick="CKEDITOR.tools.callFunction('+clickFn+', \'ol\')" hidefocus=true data-l="ol" style="float: left;outline: none;" title="'+editor.lang.list.numberedlist+'"><span class="cke_button_icon cke_button__numberedlist_icon" style="'+olstyle+'"></span></a>';return reHtml}
